  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        maxWidth: wide ? 760 : 540, maxHeight: "90vh",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `linear-gradient(135deg, ${G.pale}, ${G.wash})`,
        display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ flex: 1, minWidth: 200 }}>
        <div style={{ fontWeight: 700, color: G.dark, fontSize: 14, marginBottom: 4 }}>
          {cert.title ?? cert.certificate_number ?? "Certificate"}
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `linear-gradient(135deg, ${G.wash}, ${G.cream})`,
        display: "flex", alignItems: "center", justifyContent: "center",
...
export default function CertificatesPage() {
  const [tab,        setTab]        = useState("certificates"); // "certificates" | "templates"
  const [certs,      setCerts]      = useState([]);
  const [templates,  setTemplates]  = useState([]);
  const [loading,    setLoading]    = useState(true);
  const [modal,      setModal]      = useState(null); // "add-cert"|"edit-cert"|"view-cert"|"add-tmpl"|"edit-tmpl"
  ...
  const toggleRevoke = async (cert) => {
    const msg = cert.is_revoked ? "Restore this certificate?" : "Revoke this certificate?";
    if (!window.confirm(msg)) return;
    await supabase.from("certificates").update({ is_revoked: !cert.is_revoked }).eq("id", cert.id);
    fetchCerts();
  };
  ...
  // ── Open helpers ─────────────────────────────────────────────
  const openAddCert = () => {
    setForm({ issued_at: new Date().toISOString().slice(0, 10) });
    setError(null); setModal("add-cert");
  };

  const openEditCert = (c) => {
    setForm({
      id: c.id, recipient_name: c.recipient_name, title: c.title,
      certificate_number: c.certificate_number, certificate_type: c.certificate_type,
      issued_at:   c.issued_at   ? c.issued_at.slice(0, 10)   : "",
      expiry_date: c.expiry_date ? c.expiry_date.slice(0, 10) : "",
      is_revoked: c.is_revoked, notes: c.notes, template_id: c.template_id,
    });
    ...
    <div style={{ padding: "28px 32px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: G.base, margin: "4px 0 0", fontSize: 14 }}>
            Issue and manage GAD certificates and templates.
          </p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          {tab === "certificates" && (
            <Btn onClick={openAddCert}>+ Issue Certificate</Btn>
          )}
          {tab === "templates" && (
            <Btn onClick={() => { setForm({}); setError(null); setModal("add-tmpl"); }}>+ New Template</Btn>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: `2px solid ${G.wash}`, paddingBottom: 0 }}>
        {[["certificates", "🏅 Certificates"], ["templates", "📄 Templates"]].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} style={{
            ...
            color:        tab === id ? G.dark  : G.light,
            borderBottom: tab === id ? `2px solid ${G.dark}` : "2px solid transparent",
            marginBottom: -2,
          }}>{label}</button>
        ))}
      </div>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
            <input
              ...
              style={{
                ...
                fontSize: 13, fontFamily: "'DM Sans', sans-serif", background: G.white,
                color: G.dark, outline: "none", width: 300,
              }}
            />
            <select
          ...
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "60px 40px", color: "#aaa" }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 600, color: G.dark, marginBottom: 8 }}>
                {search || filterType ? "No certificates match your filters." : "No certificates issued yet."}
              </div>
              {!search && !filterType && <Btn onClick={openAddCert}>+ Issue Certificate</Btn>}
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {filteredCerts.map(c => (
                <CertCard
                  ...
                  onEdit={openEditCert}
                  onDelete={deleteCert}
                  onView={cert => { setForm(cert); setModal("view-cert"); }}
                />
              ))}
            </div>
          )}
      ...
      {tab === "templates" && (
        <>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "60px 40px", color: "#aaa" }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>📄</div>
              <div style={{ fontSize: 16, fontWeight: 600, color: G.dark, marginBottom: 8 }}>No templates yet.</div>
              <Btn onClick={() => { setForm({}); setError(null); setModal("add-tmpl"); }}>+ New Template</Btn>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {templates.map(t => (
                <TemplateCard
                  key={t.id} tmpl={t}
                  onEdit={tmpl => { setForm({ ...tmpl }); setError(null); setModal("edit-tmpl"); }}
      ...
      {modal === "view-cert" && form && (
        <Modal title="Certificate Details" onClose={() => setModal(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{
              background: `linear-gradient(135deg, ${G.cream}, ${G.wash})`,
              border: `2px solid ${G.pale}`, borderRadius: 14,
              padding: "28px 32px", textAlign: "center",
            }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}>🏅</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: G.dark, marginBottom: 4 }}>
                {form.title ?? "Certificate of Completion"}
              </div>
              <div style={{ fontSize: 13, color: G.base, marginBottom: 16 }}>This is to certify that</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, fontWeight: 800, color: G.mid, marginBottom: 16 }}>
                {form.recipient_name}
              </div>
            ...
            {form.notes && (
              <div style={{ fontSize: 13, color: G.base, background: G.cream, borderRadius: 8, padding: "10px 14px" }}>
                📝 {form.notes}
              </div>
            )}
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <Btn variant="danger" small onClick={() => { toggleRevoke(form); setModal(null); }}>
                {form.is_revoked ? "Restore" : "Revoke"}
              </Btn>
      ...
      {(modal === "add-cert" || modal === "edit-cert") && (
        <Modal title={modal === "add-cert" ? "Issue Certificate" : "Edit Certificate"} onClose={() => setModal(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Input label="Recipient Name" value={form.recipient_name}
              onChange={v => setForm(f => ({ ...f, recipient_name: v }))} required />
            <Input label="Certificate Title" value={form.title}
              onChange={v => setForm(f => ({ ...f, title: v }))}
              placeholder="e.g. Certificate of Completion" />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Input label="Certificate Number" value={form.certificate_number}
                onChange={v => setForm(f => ({ ...f, certificate_number: v }))}
                placeholder="e.g. GADRC-2024-001" />
              <Select label="Type" value={form.certificate_type || ""}
                onChange={v => setForm(f => ({ ...f, certificate_type: v }))}
                options={[{ value: "", label: "— select type —" }, ...CERT_TYPE_OPTIONS]} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Input label="Issue Date" type="date" value={form.issued_at}
            ...
            <Select label="Template" value={form.template_id || ""}
              onChange={v => setForm(f => ({ ...f, template_id: v || null }))}
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: G.dark, cursor: "pointer" }}>
              <input type="checkbox" checked={!!form.is_revoked}
                onChange={e => setForm(f => ({ ...f, is_revoked: e.target.checked }))}
