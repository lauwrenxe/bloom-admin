...
function SectionTitle({ children }) {
  return (
    <h2 style={{
      ...
      fontWeight: 700, color: G.dark, margin: "0 0 16px 0",
    }}>{children}</h2>
  );
}
    ...
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ background: G.wash, borderRadius: 6, height: 8 }}>
        <div style={{
          ...
          background: color || G.base, transition: "width .7s ease",
        }} />
      </div>
    </div>
  );
...
function EmptyState({ emoji, message }) {
  return (
    <div style={{ textAlign: "center", padding: "28px 16px", color: G.light, fontSize: 13 }}>
...
export default function AnalyticsPage() {
  ...
  const fetchModuleStats = async () => {
    try {
      ...
      try {
        ...
        (prog ?? []).forEach(p => {
          if (map[p.module_id]) {
            map[p.module_id].total++;
            if (p.status === "completed") map[p.module_id].completed++;
          }
        });
        setModuleStats(Object.values(map).map(m => ({
          title: m.title, total_students: m.total, completed_students: m.completed,
        })));
      } catch (_) {}
    }
  };

  // ── Seminar attendance stats ──────────────────────────────────
  const fetchSeminarStats = async () => {
    try {
      ...
      try {
        ...
        (regs ?? []).forEach(r => {
          if (map[r.seminar_id]) {
            map[r.seminar_id].registered++;
            if (r.status === "attended") map[r.seminar_id].attended++;
          }
        });
        setSeminarStats(Object.values(map).map(s => ({
          title: s.title, total_registered: s.registered, total_attended: s.attended,
        })));
      } catch (_) {}
    }
  };
  ...
  const fetchCertStats = async () => {
    try {
      ...
      if (error) { console.warn("Cert stats 400 — column issue:", error.message); setCertStats({}); return; }
  ...
  const fetchAssessStats = async () => {
    // Table name unknown — skip gracefully, show empty state
    setAssessStats({});
  };
  ...
  const certSegments = Object.entries(certStats.byType ?? {}).map(([type, value]) => ({
    label: type, value, color: certTypeColors[type] ?? "#888",
  }));
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 360, flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ color: G.base, fontSize: 15 }}>Loading analytics…</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: G.base, margin: "4px 0 0", fontSize: 14 }}>
            Live insights across modules, seminars, assessments, and students.
          </p>
        </div>
        <button
      ...
      {/* ── Row 1: Module Completion + Seminar Attendance ── */}
          ...
          {seminarStats.length === 0 ? (
            ...
            seminarStats.slice(0, 6).map((s, i) => {
              ...
              return (
                <MiniBar
                  key={s.seminar_id ?? i}
                  label={s.title ?? s.seminar_title ?? "Untitled"}
                  value={`${pct}%`}
                  max={100}
                  color={pct >= 75 ? G.mid : pct >= 40 ? G.base : G.light}
      ...
      {/* ── Row 2: Assessment Stats + Certificate Breakdown ── */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <Card>
          ...
          {certStats.total === 0 || !certStats.total ? (
      ...
      <Card>
        ...
        {leaderboard.length === 0 ? (
          <EmptyState emoji="🏆" message="No badge data yet." />
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {leaderboard.map((row, i) => (
              <div key={row.user_id} style={{
                background: i === 0 ? `linear-gradient(135deg, ${G.wash}, ${G.cream})` : G.cream,
                border: i === 0 ? `2px solid ${G.pale}` : `1px solid ${G.wash}`,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    fontWeight: 700, color: G.dark, fontSize: 13,
