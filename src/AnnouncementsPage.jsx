...
const EDGE_FN_URL = "https://vfpgzuehfebhawlidhsz.supabase.co/functions/v1/send-announcement-email";
      ...
      {showNotify && notifyTarget && (
        <div style={s.overlay}>
          <div style={{ ...s.modal, maxWidth: 520 }}>
            <div style={{ padding: "18px 24px", background: `linear-gradient(135deg,${G.dark},${G.base})`, borderRadius: "16px 16px 0 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ ...s.mBody, background: G.wash }}>
              <div style={{ background: "#fff", border: `1px solid ${G.pale}`, borderRadius: 8, padding: "12px 16px", marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12, color: "#666", marginTop: 4, lineHeight: 1.5, maxHeight: 60, overflow: "hidden" }}>
                  {(notifyTarget.body || notifyTarget.content || "").substring(0, 150)}
                  {(notifyTarget.body || notifyTarget.content || "").length > 150 ? "…" : ""}
                </div>
              </div>
