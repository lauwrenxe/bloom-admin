...
export default function AnnouncementsPage() {
  ...
  const [filterStatus,  setFilterStatus]  = useState(""); // ""|"active"|"draft"|"expired"
  ...
  const filtered = announcements.filter(a => {
    ...
    const matchStatus = !filterStatus ||
      (filterStatus === "active"  &&  isActive(a)) ||
      (filterStatus === "draft"   && !a.is_published && !isExpired(a.expires_at)) ||
      (filterStatus === "expired" &&  isExpired(a.expires_at));
  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: G.base, margin: "4px 0 0", fontSize: 14 }}>
            Manage announcements visible to GADRC students.
          </p>
        </div>
        <Btn onClick={() => { setForm({ priority: "normal", type: "general", is_published: false }); setError(null); setModal("add"); }}>
          + New Announcement
        </Btn>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
        {[
          { label: "Total",   value: announcements.length, emoji: "📢" },
          { label: "Active",  value: activeCount,          emoji: "✅" },
          { label: "Draft",   value: draftCount,           emoji: "📝" },
          { label: "Expired", value: expiredCount,         emoji: "⏳" },
        ].map(({ label, value, emoji }) => (
          <div key={label} style={{
            background: G.white, border: `1px solid ${G.pale}`,
            borderRadius: 12, padding: "12px 18px", minWidth: 100,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtered.map(a => (
            <div key={a.id} style={{
              ...
              opacity: isExpired(a.expires_at) ? 0.6 : 1,
            }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexShrink: 0, alignItems: "center" }}>
                  <Btn small variant={a.is_published ? "danger" : "secondary"}
        ...
        <Modal title="Announcement" onClose={() => setModal(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {!form.is_published
                ? <Tag color="gray">Draft</Tag>
                : isExpired(form.expires_at)
                  ? <Tag color="yellow">Expired</Tag>
                  : <Tag color="green">Active</Tag>
              }
            </div>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, color: G.dark, margin: 0 }}>
              {form.title}
            </h2>
            {form.content && (
              <div style={{
                fontSize: 14, color: G.dark, lineHeight: 1.6,
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Select label="Priority" value={form.priority || "normal"}
                onChange={v => setForm(f => ({ ...f, priority: v }))}
                options={PRIORITY_OPTIONS} />
              <Select label="Type" value={form.type || "general"}
                onChange={v => setForm(f => ({ ...f, type: v }))}
                options={TYPE_OPTIONS} />
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Input label="Expiry Date" type="date" value={form.expires_at}
                onChange={v => setForm(f => ({ ...f, expires_at: v }))} />
              <Input label="Target Roles" value={form.target_roles}
                onChange={v => setForm(f => ({ ...f, target_roles: v }))}
                placeholder="e.g. students, all" />
            </div>
